...
--generate)
	...
	echo "nqn.2021-06.org.rapiddisk:$(hostname)-$( (
		echo "obase=16"
		echo $((10000 + RANDOM % 100000))
	) | bc)"
	;;
--get)
...
--set)
	...
	if [ -e /etc/nvme/hostnqn ] && [ ${force} -ne 1 ]; then
		...
		# shellcheck disable=SC2004
		NQN="nqn.2021-06.org.rapiddisk:$(hostname)-$( (
			echo "obase=16"
			echo $((10000 + RANDOM % 100000))
		) | bc)"
		if [ ! -d /etc/nvme ]; then
			if ! mkdir -p /etc/nvme; then
				echo -e "Error. Unable to create the /etc/nvme directory."
				exit 1
			fi
		fi
		if ! echo "${NQN}" >/etc/nvme/hostnqn; then
